...
def open_dashboard_window():
    ...
    # Create a brand new window on top of the root
    dashboard_win = tk.Toplevel(root)
    dashboard_win.title("Rental Dashboard")
    dashboard_win.geometry("300x350")

    tk.Label(root, text="Enter Rental Info", font=("Arial", 18, "bold")).pack(pady=20)
    
    tk.Label(root, text="Title").pack()
    rental_title = tk.Entry(root, width=30)
    rental_title.pack(pady=5)

    tk.Label(root, text="Description").pack()
    rental_description = tk.Entry(root, width=30)
    rental_description.pack(pady=5)

    tk.Label(root, text="Feature").pack()
    rental_feature = tk.Entry(root, width=30)
    rental_feature.pack(pady=5)

    tk.Label(root, text="Price").pack()
    rental_price = tk.Entry(root, width=30)
    rental_price.pack(pady=5)
    
    tk.Button(root, text="Submit", command=handle_insert_rental).pack(pady=20)
